...
2.  Add the zod-prisma generator to your schema.prisma
    ...
    generator zod {
      ...
      imports                  = null // (default) will import the referenced file in generated schemas to be used via imports.someExportedVariable
			...
			zodPath                  = "zod" // (default) Path that will be used for importing "z" variable (import * as z from "${zodPath}")
...
### Custom zod package / path

Sometimes you may want to use custom `zod` with added / extended schemas. It can be located inside your project or in `node_modules` (installed from the package registry).

To use your custom methods and schemas, you'll need to override default `zodPath`:

```prisma
generator zod {
  provider      = "zod-prisma"
  output        = "./zod"
  zodPath       = "nestjs-zod/z"
}

model User {
  birthDate	String /// @zod.dateString().past().minYear(1920)
}
```
